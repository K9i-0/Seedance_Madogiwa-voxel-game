...
3. `wan-video`では音声同期方式を確定し、採用MP4と修復音声を作る。動画の誤読をピンポイント修正する場合は[動画内音声を参照するIrodori部分修復](../wan-video/references/video-voice-patch.md)を読み、同一話者の動画音声を参照して一文生成→必要部分だけ切り出す方法を第一候補にする。音声修正後のMP4をRemotionの入力にする。
