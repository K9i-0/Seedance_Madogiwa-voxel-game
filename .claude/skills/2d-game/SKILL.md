...
「オープニング動画 → ゲーム開始 → イベント動画 → ゲームの続き」のように、Seedance製の動画クリップをカットシーンとして挟める2Dゲームを作る。動画はユーザーがCapCut（Seedance 2.0）で制作して**添付ファイルとして渡してくる**。動画が届く前でもゲーム本体は先に作れる設計にする（後述のとおり、未配置の動画は自動スキップされる）。
...
- 世界観の基本資料: `01_WORLD/WORLD_BIBLE.md`（ユーザー指定がない場合の発想材料。改変可能）
- 過去エピソード台帳: `01_WORLD/STORY_TIMELINE.md`（ネタの参照用。連続性や整合性を要求しない）
- キャラクター設定: `02_CHARACTERS/*.md`（各キャラの人物同一性だけをゲーム内でも維持する）
...
- 2D用アート素材:
  ...
  - インラインSVGでキャラを描く場合の手本: `08_ROMANCE_NOVEL_GAME/src/avatar.ts`（人物同一性の識別要素を確認してSVG化する）
...
- 唯一のハード条件は、登場キャラクターを同一人物・同一キャラクターとして識別できること。実写メンバーは正典写真の顔、非実写キャラクターは各ファイルの「人物同一性」に記載した識別要素を維持する。
- 衣装、小道具、表情、ポーズ、性格、役割、能力、ジャンル、結末、暴力・恐怖・風刺の強度、過去作との整合性、夢オチ、復活、再登場はユーザー指定に従う。Story Formulaや笑顔の結末を自動適用しない。
- パロディ・オマージュでは参照元を一律に抽象化せず、構図、演出、意匠、UI、ゲームルールをどの程度近づけるかはユーザー指定を正本にする。権利上の懸念を理由に創作意図を自動で弱めない。
