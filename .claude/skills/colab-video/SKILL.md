...
- ノートブックは**どちらのGPUでも設定変更なしで動く**（セル2が世代からバリアントを自動選択、セル6がworkflowを書き換え）。
- **Driveの重みは「共通分（エンコーダ＋VAE、約33GB）＋今のGPUのユニットペア（42GB）」の約75GBだけを保つ**運用（Google One 100GBに収まる）。GPUを切り替えたセッションでは、セル4が「必要な重みがDriveに無く旧バリアントがある」ことを検出して**旧ペアを自動削除→新ペアをDL＆Drive保存**する（切替コストは42GBのDL≒10〜20分。頻繁に往復する使い方には向かないので、ランの途中でGPUを変えない）。

- モデルは`Comfy-Org/MiniMax-H3`（非ゲート公開リポジトリ）から取得するため**HFアカウント・トークンは不要**。ゲート化されていたらノートブックがエラーを出すので、その場合のみHFトークンを用意する。
- **毎セッション重みを再ダウンロードする**（約60GB・15〜30分。Colabのディスクはセッション終了で消える）。HFからのDLはタイムアウト警告→レジュームを繰り返すことがある（`.incomplete`のサイズが伸びていれば正常）。完全に停止する場合はaria2（`-c -x16`）で`resolve/main`のURLから最終配置先へ直接落とすフォールバックがある。
...
- GPUの割当は「ランタイム → ランタイムのタイプを変更」で T4（無料）/ L4（課金）を選ぶ。
...
zipは通常20〜200MB程度。受け渡しはノートブックのセル5で「ブラウザから直接アップロード」または「Google Driveに置いてパスを指定」のどちらでもよい。

- workflow JSONの重み名はどのGPU向けでもよい（ノートブックのセル6が、割り当てられたGPUに合う重み名へ自動で書き換え、SaveVideoの`codec`も補完する）。
...
| セル | 内容 |
|---|---|
| 1 | **設定（毎セッションここだけ編集）**: `CHAPTERS`・バンドル/重み/出力のDriveパス・`NEED_I2V`/`NEED_R2V`・`COMFY_FLAGS` |
| 2 | 環境チェック（Turing以下は拒否、GPU世代→重みバリアント自動選択、ディスク見積り警告） |
| 3 | ComfyUIインストール＋aria2導入＋H3ノード確認 |
| 4 | **重み配置**: Drive優先（symlink直参照＝ディスク消費ゼロ・サイズ照合で不完全ファイル検出）→無い分は**aria2（16並列・レジューム対応・進捗表示）**でDL→Driveへ保存 |
| 5 | バンドルzip投入→展開→`ComfyUI/input/`へコピー |
| 6 | workflowをこのGPUの重み名に調整＋`codec`補完 |
| 7 | ComfyUI起動（OOM時は`COMFY_FLAGS=["--lowvram"]`にしてここから再実行） |
| 8 | **チャプター生成**: 生成済みスキップ＝中断・再開に強い。`OUT_DRIVE_DIR`設定時は**完了ごとに即Drive退避** |
| 9 | 成果物の回収（zip→ブラウザDL） |
| 10 | （任意）アドホック生成 — 画像・音声・プロンプト直指定で1本 |
...
- **HF公式クライアントは大容量DLが不安定**（タイムアウト→レジュームの繰り返し、終盤の速度低下）。ノートブックはaria2（`-c -x16`）を既定にした。中断してもセル4の再実行でレジュームされる。
...
1. ランタイムをL4にしてセル1（設定）→2〜7を実行。Drive重み・Driveバンドル運用ならセットアップは数分で終わる。
2. **パイロット必須**: セル1の`CHAPTERS`を**最初のセリフ（R2V）チャプター1本だけ**にしてセル8を実行し、チェックリスト（wav駆動・正しい話者のリップシンク・キーフレームアンカー・NG要素・尺）を全部確認してから、`CHAPTERS`を残りに書き換えてセル8を再実行する（生成済みは自動スキップ）。
3. 尺の長いランは**1セッション2〜3チャプター**に区切る。`OUT_DRIVE_DIR`を設定しておけば完了ごとに自動でDriveへ退避される（未設定ならセル9でこまめに回収。セッションが落ちると未回収のmp4は消える）。
