...
| 7 | **チャプター生成**: 冒頭でセル3のバックグラウンド重み配置の完了を待ち、**同じユニットを使うチャプターが連続するよう生成順を並べ替え**（21GBのユニット入れ替え回数を最小化。同一モード内は番号順）、使用ユニットを**使用直前にDriveからローカル化**（ディスクが足りなければ他方のユニットを自動退避）。生成済みスキップ＝中断・再開に強い。`OUT_DRIVE_DIR`設定時は**完了ごとに即Drive退避**。`AUTO_DISCONNECT=True`なら全チャプター完了後にDriveへ書き切ってから**ランタイムを自動削除**（課金停止。生成失敗時は診断のため残る） |
| 8 | 成果物の回収（zip→ブラウザDL） |
| 9 | （任意）アドホック生成 — 画像・音声・プロンプト直指定で1本 |

**課金の実態（重要）**: CUは「GPUランタイムが接続されている時間」で消費される。**セルが動いていない待ち時間（アップロード待ち・放置）も課金対象。** 対策: (1) バンドルzipと実行計画を手元に揃えてからL4に接続する、(2) 重みと成果物はDrive運用にして毎セッションのDL/回収時間を最小化する、(3) 成果物を回収したら即「ランタイム → ランタイムを接続解除して削除」で課金を止める（セル1の`AUTO_DISCONNECT=True`にすれば、Drive退避運用の本番ランでは全チャプター完了後の削除を自動化できる。パイロット中はFalseのままにする）。
...
4. 生成中はブラウザを放置しない（無料・Pro問わず約90分アイドルで切断。Colabのタブを開いたままにする）。用が済んだら**即ランタイムを削除して課金を止める**（`OUT_DRIVE_DIR`＋`AUTO_DISCONNECT=True`の本番ランなら完了時に自動で削除される。放置ランと相性がよい）。
