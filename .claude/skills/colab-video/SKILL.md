...
| 1 | **設定＋環境チェック（毎セッションここだけ編集）**: `CHAPTERS`・`EXPECTED_GPU`・`WORKERS`・Driveパス・`NEED_I2V`/`NEED_R2V`等。実行すると**実際に割り当てられているGPUを正として**重みバリアントを自動選択・表示し、`WORKERS`から各ワーカーのVRAM上限（`--reserve-vram`）を決める。`EXPECTED_GPU`と違えばここで停止 |
...
| 7 | **チャプター生成**: 冒頭でセル3のバックグラウンド重み配置の完了を待ち、**同じユニットを使うチャプターをグループにまとめ**（21GBのユニット入れ替え回数を最小化。同一グループ内は番号順）、グループのユニットを**流す直前にDriveからローカル化**（ディスクが足りなければ他方のユニットを自動退避）してから**`WORKERS`本ずつ同時実行**（出力は`[w0]`/`[w1]`のワーカー名付きで流れる）。生成済みスキップ＝中断・再開に強い。`OUT_DRIVE_DIR`設定時は**完了ごとに即Drive退避** |
| 8 | 成果物の回収（zip→ブラウザDL） |
