...
- **推奨: Google One 100GB（月¥290）＋`WEIGHTS_DRIVE_DIR`**（セル3）。重み一式（約75GB）をDriveに永続化すると、①HFからの再ダウンロードが消滅（Drive→ローカルのコピーは合計10〜20分でHFより安定）、②初回のDrive配置は無料CPUランタイムでできるのでCU消費ゼロ。**注意（2026-08実測）: Drive上のファイルへのsymlink直参照は実行時に使えない**（ComfyUIのcomfy_aimdoがFUSE越しのスライス読みに失敗し`HostBuffer.read_file_slice failed`になる）。Driveはあくまでキャッシュで、**実行前に必要な重みをローカルへコピーする**（エンコーダ・VAEはセル3で、ユニットはセル7が使用直前に自動ローカル化。L4のディスク65GBでも「共通33GB＋ユニット1本21GB≒54GB」で収まるよう、他方のユニットは自動退避される）。
...
| 3 | **重み配置**: Drive優先（エンコーダ・VAEはローカルへコピー、ユニットはセル7で遅延ローカル化。サイズ照合で不完全ファイル検出）→無い分は**aria2（16並列・レジューム・進捗表示）**でDL→Driveへ保存。**GPU切替時は容量が足りなければDrive上の旧バリアントを自動削除** |
| 4 | バンドルzip投入→展開→`ComfyUI/input/`へコピー |
| 5 | workflowをこのGPUの重み名に調整＋`codec`補完 |
| 6 | ComfyUI起動（OOM時は`COMFY_FLAGS=["--lowvram"]`にしてここから再実行） |
| 7 | **チャプター生成**: 各チャプターの使用ユニットを**使用直前にDriveからローカル化**（ディスクが足りなければ他方のユニットを自動退避）。生成済みスキップ＝中断・再開に強い。`OUT_DRIVE_DIR`設定時は**完了ごとに即Drive退避** |
| 8 | 成果物の回収（zip→ブラウザDL） |
