...
- **Driveの重みは「共通分（エンコーダ＋VAE、約33GB）＋今のGPUのユニットペア（42GB）」の約75GBだけを保つ**運用（Google One 100GBに収まる）。GPUを切り替えたセッションでは、セル3が「必要な重みがDriveに無く旧バリアントがある」ことを検出して**旧ペアを自動削除→新ペアをDL＆Drive保存**する（切替コストは42GBのDL≒10〜20分。頻繁に往復する使い方には向かないので、ランの途中でGPUを変えない）。
