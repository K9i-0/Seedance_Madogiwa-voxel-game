...
| 7 | **チャプター生成**: 冒頭でセル3のバックグラウンド重み配置の完了を待ち、**同じユニットを使うチャプターが連続するよう生成順を並べ替え**（21GBのユニット入れ替え回数を最小化。同一モード内は番号順）、使用ユニットを**使用直前にDriveからローカル化**（ディスクが足りなければ他方のユニットを自動退避）。生成済みスキップ＝中断・再開に強い。`OUT_DRIVE_DIR`設定時は**完了ごとに即Drive退避** |
| 8 | 成果物の回収（zip→ブラウザDL） |
