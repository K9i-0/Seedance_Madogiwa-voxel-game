...
| 段階 | 必要なもの | 費用 | できること |
|---|---|---|---|
| **L4生成（単発）** | Pay As You Go 100CU購入 | ¥1,179（単発） | L4（VRAM 実測22GiB / RAM 約53GB）で実チャプター生成。目安2〜3CU/時・**動画1秒あたり約1.9分**（蒸留8step・2026-09実測。20stepだった頃は約4.5分） |
...
| 4 | バンドルzip投入→展開→`ComfyUI/input/`へコピー。**workflowが参照する画像・音声が実際に配備できたかをここで検証する**（不足があると重みDL完了後に `Invalid image file` で落ちるため早期に止める）。検証は**`CHAPTERS`指定時はその範囲だけ**を見る — 生成しないチャプターの素材不足で止める理由は無い（キーフレーム未生成のチャプターが残るラン・モード別ノートブック・1本だけ回すA/Bベンチで、無関係な不足に足を取られていた） |
...
| 7 | **チャプター生成**: 冒頭でセル3のバックグラウンド重み配置の完了を待ち、**同じユニットを使うチャプターをグループにまとめ**（21GBのユニット入れ替え回数を最小化。同一グループ内は番号順）、グループのユニットを**流す直前にHFからDL**（`WEIGHTS_DRIVE_DIR`設定時はDriveからコピー。ディスクが足りなければ他方のユニットを自動退避。**削除した旧unetをComfyUIがmmapで掴んでいて空きが戻らないときは、自動でComfyUIを再起動して解放してからやり直す** — 2026-08実測）してから**`WORKERS`本ずつ同時実行**（出力は`[w0]`/`[w1]`のワーカー名付きで流れる）。生成済みスキップは**そのチャプターを流す直前に毎回**（実行開始時の一括判定ではない）**ローカルと`OUT_DRIVE_DIR`上のmp4の両方**を見る＝セッションを跨いだ再開で退避済みチャプターを再生成しないのに加え、長時間ランの途中で別セッション・手動コピーが先に完成させたチャプターも上書きしない。グループ開始前の判定は「そのグループのために21GBのunetを落とすか」の下見のみ。`OUT_DRIVE_DIR`設定時は**完了ごとに即Drive退避**。`AUTO_SHUTDOWN=True`なら全チャプター成功＋Drive退避を確認後に**ランタイムを自動削除** |<br>**`TURBO_8STEP=True`（既定）のとき、生成の直前にworkflowへステップ蒸留を適用する**: 対象チャプターのユニットに応じたTurbo LoRA（1.4GB）をDLし、`LoraLoaderModelOnly`＋`MiniMaxH3SigmaShift(12/3)`を差し込んで`euler`/`beta`/8stepにする。ノードのクラス名・入力名は起動済みComfyUIの`/object_info`で確かめてから触り、名前が変わっていれば実機の定義を添えて止まる（`TURBO_8STEP=False`で従来の20stepに戻せる）。適用済みworkflowは再実行しても二重適用しない
| 8 | 成果物の回収（zip→ブラウザDL） |
...
### ステップ蒸留8step（`TURBO_8STEP` — 既定で有効・2026-09採用）

**20step → 8step で実測x2.34。** セル1の`TURBO_8STEP=True`（既定）でセル7が自動適用する。

計測（2026-09・L4+sage・`83_fukuchan_yotan_interview` ch3 R2V 158f＝6.6秒・同一シード）:

| arm | steps | 分 | s/step | vs base | 添付wavとの包絡一致 | baseとの映像差 PSNR/SSIM |
|---|---:|---:|---:|---:|---:|---|
| base（従来） | 20 | 37.5 | 101.9 | x1.00 | 0.95 | （基準） |
| **turbo8（採用）** | 8 | **16.0** | 105.5 | **x2.34** | **0.97** | 19.4 / 0.708 |
| pdd8（不採用） | 8 | 17.8 | 104.5 | x2.11 | **0.84** | 17.5 / 0.644 |

- **1stepの秒数は3armともほぼ同じ**（101.9 / 105.5 / 104.5）。差は純粋にstep数（20→8）から来ている＝
  蒸留が1stepを重くする副作用は無い。
- **品質はユーザーの目視で3armとも同等**。自動指標でもturbo8はbaseに最も近く、
  **添付wavとの包絡一致は0.95→0.97とむしろ上がった**（wavをAS-ISで使い続けている）。
- **pdd8を採らなかった理由**: 速度で劣り（x2.11）、公式PDD用のカスタムノードが要り、
  包絡一致が0.95→**0.84**と落ちた（目視では問題なかったが、指標が悪化した方をわざわざ採る理由が無い）。
- PSNRの平均と最小が近い（19.4/18.7）＝**差は全編に一様**で、特定フレームだけ破綻しているのではない。
- ラン全体への効き（83ラン・13チャプター・50秒）: **5.1時間・¥120〜180 → 2.5時間・¥60〜90**。

採用の条件と注意:

- LoRAは**ベンチした版に固定**してある（`minimax_h3_{fl2v,ref2v}_turbo_8step_v1.0_768p_comfyui_bf16.safetensors`
  ＠`lightx2v/Minimax-h3-Turbo`）。版を上げるときは**必ずベンチで測り直してから**セル7の`TURBO_LORA`を変える。
- レシピも測ったとおりに固定: `euler` / `beta` / 8step / `MiniMaxH3SigmaShift` video 12.0・audio 3.0。
  **サンプラーとshiftは蒸留の訓練条件**なので勝手に変えない。
- **新しいランでは従来どおりパイロット1本で声とリップシンクを確認してから残りを流す。**
  1チャプター1シードで測った結果なので、話者・尺・構図が変わったランでも同じとは限らない。
- 戻し方は`TURBO_8STEP=False`（セル1）だけ。バンドルもworkflowも作り直さなくてよい。

### 生成方式のA/Bベンチ（`h3_ab_bench_colab.ipynb`）

上のセル10が計測するのは**起動フラグ**だけ。**生成方式そのもの**（ステップ蒸留LoRA・sparse attention・重みのバリアント）を
同一チャプター・同一シードで比べて、**速度と精度の両方**を1枚のレポートにするのが本ノートブック。
「20ステップのまま回し続けてよいか」を判断するための道具で、採用を決めるまで本番の構成は変えない。

**作り方**（Claudeが実行する）:

```
python3 .claude/skills/colab-video/build_ab_bench_notebook.py 03_SCRIPTS/<NN>_<slug>
```

正典`h3_colab.ipynb`のセル1〜9をそのまま取り込み（重み配置・ディスク管理・生成・Drive退避の機構を再利用するため）、
末尾にベンチ3セル（11=設定 / 12=実行 / 13=レポート）を足した`<ランディレクトリ>/h3/h3_ab_bench_colab.ipynb`を作る。
`--chapter ch8` / `--arms base,pdd8,sparse` で明示指定もできる。**正典を更新したらこのスクリプトを再実行**して反映させる。

対象チャプターの選び方（スクリプトが自動で行い、全チャプターの素材チェック表を出す）:

1. **入力ファイル（キーフレーム・シート・wav）が実際に揃っているチャプターだけを候補にする。** 素材不足は
   **ローカルで**弾く — Colabへ上げてDriveをマウントしてから落ちるのは、GPU課金を払ってから気付くということなので。
   明示指定したチャプターが揃っていなければ、不足ファイル名を挙げて止まる
2. その中から**R2V（セリフあり）を優先する** — wav駆動リップシンクの劣化がこのパイプラインの最大の関心事だから。
   揃ったR2Vが無ければI2Vで組み、「**この構成ではリップシンクは測れない**」と警告する
3. 選んだチャプターが160フレームより長ければ、**計測用に124フレーム（約5.2秒）へ切る**（セル11の`BENCH_FRAMES`）。
   全armで同じ尺なので比較は成立する。`0`にすれば台本どおりの尺に戻る。
   ⚠ **sparse armの効きは系列長に比例して大きくなる**ので、sparseの採用可否まで詰めるときは`BENCH_FRAMES=0`で測り直す

⚠ **キーフレームは直列チェーン**（各フレームが次の種になる・`gen_keyframes.sh`）なので、途中の1枚だけを作り直すことはできない。
素材の欠けたランでセリフありチャプターをベンチしたいなら、チェーンを通しで流し直すか、素材の揃った別のランを指定する。

**比較できるarm**（セル11の`BENCH_ARMS`。`base`は常に基準として先頭に入る）:

| arm | 中身 | 追加DL |
|---|---|---|
| `base` | **現行の本番設定**（そのGPUの既定重み・`res_multistep`/`simple`・20step） | — |
| `pdd8` / `pdd4` | 公式PDD Acc（alibaba-pai）8step / 4step。専用ノード＋`euler`＋訓練済みsigma＋CFG1 | node + 1.4GB |
| `turbo8` | lightx2v Turbo 8step v1.0 768p LoRA（素のLoRA・`euler`/`beta`・shift 12/3） | 約1.4GB |
| `sparse` | ComfyUI core の Block Sparse Attention（20step据え置き・attentionだけ疎に） | — |
| `enc_nvfp4` | テキストエンコーダを`nvfp4_awq`（14.6GB）へ。既定の`int8_convrot`は約32GB | 14.6GB |
| `unet_int8` | UNETを`int8_convrot`へ（**L4の既定は`fp8_scaled`**なのでAdaでのみ意味がある。A100は既定が同じ） | 21GB |
| `best` | `pdd8` + `enc_nvfp4` + `sparse` の合成（採用候補の最終確認） | 上記合計 |

**回し方**:

1. セル11で`BENCH_ARMS`を決め、**セル12をそのまま実行する**。既定`BENCH_DRY_RUN=True`＝**生成せず**に
   「各armのworkflowが実機のComfyUIで組めるか」を`/object_info`と突き合わせて検証し、所要時間とCU/円の見積りを出す。
   ノードのクラス名も入力名も**実機の定義から引く**ので、名前が変わっていれば実際の定義を添えて止まる（既定値のまま黙って走らない）。
2. 検証が通ったら`BENCH_DRY_RUN=False`にしてセル12を再実行 → armを順に生成する。
   成果物は`chN__<arm>.mp4`（生成済みはスキップ＝中断・再開・セッション跨ぎに強い）。
3. **セル13でレポート**。⚠ `BENCH_SHUTDOWN=True`だとセル12の最後で切断されるので、レポートは切断前に実行する。

**セル13が出すもの**（`<chapter>_ab_report.md`としてDriveへも退避。**そのままClaudeへ貼れば採用可否を相談できる**）:

- 性能: arm毎の wall分 / s per step / vs base / VRAMピーク / 円per本
- 精度（自動指標）:
  - **baseとの映像差**（PSNR平均・最小、SSIM）— 蒸留armは絵が変わって当然なので「劣化」ではなく**変化の大きさ**。最小PSNRが落ち込むフレームは破綻の疑いとして必ず目視する
  - **音声ピーク／クリップ数** — 低step蒸留で実際に報告のある劣化（6stepで−0.0dBまでクリップした第三者実測）。**base比で悪化したら不採用**
  - **添付wavとの包絡一致**（相関とオフセット）— 入力wavが素材としてそのまま使われているかの代理指標。**base比が大きく落ちたら「別の声を生成し始めた」疑い**
  - **キーフレーム追従**（先頭/末尾フレーム vs `ch*_start.png`/`ch*_end.png`のPSNR）— I2Vは厳密アンカーなので高いはず。R2Vは元々ゆるい
- 目で見る材料: **横並び動画**（全armを横に並べた無音mp4）と**コンタクトシート**（同一時刻のフレームをarm毎に1行）

**自動指標はスクリーニングであって合否ではない。** とくに**リップシンクは自動指標では判定しない**
（顔検出をしない全画面プロキシしか出していない）。合否はレポートの目視チェックリスト
（添付wavの声か／正しいキャラの口だけが鳴っている間だけ動くか／NG変更要素が崩れていないか等）と、
ローカルの`/image-validation`（Qwen3-VL）で確定させる。

**注意**:

- 重みを差し替えるarm（`enc_nvfp4`/`unet_int8`/`best`）は追加DLと入れ替えが起きる（L4のディスクは実測65GB）。
  **まずは重みを変えない`base`/`pdd8`/`turbo8`/`sparse`だけで回すのが安い。**
- 蒸留LoRAは**互いに併用できない**（PDDとlightx2vは同時に適用しない）。`euler`以外のサンプラーは訓練グリッド外を評価するので使わない。
- PDDは`SigmaShift`を**12.0/3.0ちょうど**に要求する。ベンチはSigmaShiftノードがあれば自動で挿すが、無い環境では警告して
  モデル既定のまま進む（音が破綻していたらここを疑う）。
- ベンチはバンドルの`chN_workflow.json`を書き換えて回すため、原本を`chN_workflow.orig.json`へ退避し、
  終了時に書き戻す。異常終了した場合は`.orig.json`から手で戻す。
- **採用を決めたら、実測を`H3_COLAB.md`と本スキル0章・5章へ反映してから**正典`h3_colab.ipynb`の既定を変える。
  ベンチで勝っただけでは本番構成を変えない。
