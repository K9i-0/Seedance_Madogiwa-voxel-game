...
- 管理画面: `https://madogiwa-studio.madogiwa-studio.workers.dev/admin`。Remote MCPは同じworkers.devホストの `/mcp` を使う。既存のCloudflare Access認証を維持しているため、公開ドメインへの変更だけを理由にMCP設定を置き換えない。
