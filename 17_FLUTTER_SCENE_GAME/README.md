...
- 作業台から8種類の設備・道具をクラフト
- 採取／建設を常設ボタンで切り替え、地面タップ後のメニューから即時建築
- 床・壁・屋根の個別建築に加え、2×2の小屋を一括建築可能
- 木材1個で松明を設置。開始キャンプにも検証用の松明を1本配置
- 床4枚、壁4枚、屋根、焚き火、作業台で生活基盤が完成
- 再会したメンバーは発見順にそば屋の移動軌跡をたどる勇者パーティー隊列で同行
- やめ太郎の近距離自動収集、ゆめみんの2倍ジャンプ、タコさんの夜間視界を順次解放
...
- ミニマップのそば屋は向きを示す三角形、常設HUDは目標と不足資材数を表示
- 冒険・持ち物／クラフト・設定を「島流し手帳」の3タブへ統合
- 目標更新時の達成バナーと、エンディング選択後のゲームクリア演出
