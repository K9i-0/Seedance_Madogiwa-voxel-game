...
2026-09-13: 最高画質も高画質と同じ空の間接光・露出・エフェクト強度を使う。動的GIは空の拡散光への加算ではなく置換であり、強度0.3のプローブへ切り替えると陰が沈む可能性があるため無効化した。最高画質の追加予算は影の解像度・距離とAOのサンプル数へ使う。炎や水を含め、品質段階を理由に発光・反射・不透明度を増やさない。GI再導入時は同じカメラで空の照明との明るさの連続性を確認する。
