**全4人のスマホ軽量モデルを追加（2026-09-19）。** iOS/Androidは軽量版、PCは現行版を自動選択。元モデルは保持。4人合計のゲーム変換後データは約45%減、そば屋の三角形は約51%減。顔・動作を比較済み。実機の発熱低下量は未測定。[モデル仕様・切替方法](../04_GAME_ASSETS/3d/hazard_adopted/mobile_20260919/README.md)／[検証記録](qa/mobile-models-20260919.json)。

# そば屋ハザード

**3Dモデルを軽量化（2026-09-19）。** そば屋の顔・手・動作を維持し、頭部の剛体パーツを整理して三角形を約19.9万→12.7万（36.1%減）。福ちゃんは形状・画像画素を変えずGLBを可逆圧縮・整理。全127クリップ、口パク、ジョッキ追従を保持した。[モデルと再生成](../04_GAME_ASSETS/3d/hazard_adopted/optimized_20260919/README.md)／[Mac本編・画像比較の検証](qa/model-optimization-20260919.json)。GLB容量削減とゲームの展開データ量は別。FPS・発熱改善率は未測定で、旧配布ZIP・iPhoneインストール済みアプリは未更新。

**そば屋モデル更新（2026-09-17）。** 承認済みの握り・垂直な立ちポーズ・歩走の手・3攻撃・左手挨拶を採用。待機は胸元にジョッキを構える姿勢。旧モデル用のジョッキ90度補正を外し、位置と角度の両方をソケットへ合わせる。[検証記録](qa/sobaya-motion-adoption-20260917.json)。今回の変更は開発版へ反映し、旧配布ZIPは更新していません。

**ゆめみ村の世界観へ体験版を更新（2026-09-13）。** 港→生活圏のたこさん商店→山の神社という3区画に変更。巨大そば屋の片言から会社の関与を知り、撃破後に施設を発見して終わる構成。エンジンの真相と帰還は本編へ留保する。[現行の体験版仕様](GAMEPLAY.md)／[制作ゴールと検証状況](YUMEMI_DEMO_PLAN.md)。以下は旧版を含む制作履歴。既存のMac配布物は今回の変更を含まない。
...
## 実操作の序章・救難準備（2026-09-12）

「新しく始める」は短い島流しの導入から、やめ太郎の声かけ付き研修へ進む。移動、構える、標的への実弾命中、装填完了、忍び足、発見後に遮蔽で警戒を解く、出発の7段階。研修のそば屋は本編と同じ視認・捜索判定を使い、持ち場を動かず攻撃しない。通常の敵・物資は研修中に触れず、弾薬・体力・戦績は本編開始時に初期化する。各練習にはやり直しとスキップがあり、保存からはその練習の開始状態を再開する。

本編は村の鍵→農場の救難準備→山の巨大そば屋→家内でやめ太郎・たこさんに再会→帰還。農場では工具小屋のバッテリーと納屋二階の避難者名簿を、順序自由で回収してたこさんへ届ける。任務品は持ち物の枠を消費しない。受け渡しの会話を最後まで進めると東門が開き、最高難度以外では出発用の補給を受け取る。ケースが満杯なら補給所前に置かれる。メダリオンとギャラリーは任意探索。

環境の3Dアセットは現在の区画だけを保持する。移動時は次の区画の準備が完了してから古い環境を解放し、キャラクター・共通小物と各区画の進行状態は維持する。読み込み中は操作を止め、失敗時は再試行／戻るを表示する。往復時に敵・拾得状態が復活することはない。旧セーブは研修へ戻さず、旧農場・山道セーブは救難準備済みとして継続する。

`madogiwa.gameAction(action=tutorial, step=move|aim|shoot|reload|sneak|escape|complete)` は保存を無効にした研修確認。`look, dx, dy` は通常の視点操作経路。シナリオの直接配置は攻略経路の証明とは区別する。状態テストは `test/game_tutorial_progress_test.dart`、区画寿命は `test/game_region_loader_test.dart`。
