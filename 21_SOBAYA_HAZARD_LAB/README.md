...
**アプリ名を統一（2026-09-08）。** iOS・Android・macOSの表示名を「そば屋ハザード」へ揃えた。Macのビルド成果物も「そば屋ハザード.app」となる。iOS・macOSのビルド・起動表示を確認済み。共通アイコンは[正方形の原画3案](../04_GAME_ASSETS/3d/ui/hazard/icon-concepts-20260908/README.md)を制作し、採用待ち。Android Adaptive Iconのレイヤー出力は採用案から行う。[検証記録](qa/app-branding-20260908.json)。
