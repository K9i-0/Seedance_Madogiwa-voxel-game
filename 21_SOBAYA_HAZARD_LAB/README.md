...
最新Macアプリ（ビルド `9492ff0`）: [そば屋ハザード.app](../.local/hazard_releases/sobaya-hazard-macos-9492ff0/そば屋ハザード.app) / [ZIP](../.local/hazard_releases/sobaya-hazard-macos-9492ff0.zip)。全体245テストと追加5件の回帰テスト、Mac debug／iPhoneシミュレーターの縦横UI・操作、11条件のprofileを確認。[Release同梱検査](qa/mobile-world-stealth-release-20260908.json)。スマートフォン実機の操作感・長時間性能は未検証。

**2026-09-08更新：** 確認済みの肩ウェイト・歩行・ダンスを[本編採用GLB](../04_GAME_ASSETS/3d/hazard_adopted/README.md)へ反映。福ちゃんのダッシュは既存Mixamo Run、そば屋は逃走・追跡の調整版を採用した。ジョッキ保持・3攻撃・回避など本編専用動作は保持する。未発見の通常そば屋は出現時20%でダンス3種のいずれかを選ぶ。疑い・発見・被弾で中断し、一度発見後は再開しない。ボス対象外。抽選と発見履歴はチェックポイントへ保存する。196テスト・本編の3ダンス／銃声による中断／採用Run／追跡と握り／会話の口パクを確認。[検証記録](qa/adopted-vrm-and-dance-20260908.json)。
