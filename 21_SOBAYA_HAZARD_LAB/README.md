...
最高難度では各章の全個体を撃破するまで次章へ進めず、最終章では家の玄関も開かない（前章への引き返しは可）。初期は装填2発・予備0発・ショットガン0発。やめ太郎の一度限りの補給25発（標準・気軽は10発）、各章に散在する弾は1発のみ、箱から弾は出ない。たこさんの弾薬はビールと引き換えに補充可能。通常個体HP100に対するダメージは以下。頭部は骨格、ジョッキは持ち手の変換に追従する球で判定し、ショットガンの広い胴体判定では弱点ボーナスを与えない。
...
当時の最高難度の蹴りは8ダメージで連続怯ませ不可だった（2026-09-08に操作を廃止）。無料3発だけでは倒せるのは最大1体（ビール1個で交換最低価格2個に届かない）。やめ太郎の25発込みで1面の8体に必要な最少24発を確保。2面以降は交換が必要。通常個体19体の最少57発とボス頭部へのショットガン9発に対し、初期・補給・配置30発＋弾10発×3回（ビール6個）＋散弾5発×2回（6個）で収支が成立する。これは命中を前提にした資源検証で、人間の通しプレイによる難度評価は別途必要。
