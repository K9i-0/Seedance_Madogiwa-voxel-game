...
本編は「そば屋エンジン」の放棄と、廃村ゆめみ村へ島流しされた三人を中心に再構成し、地の文の読み上げ5本と購入時の声・被弾声を含む88本の発話、探索メモ8枚、ポスター12枚の裏書きを収録。[物語・事故の因果関係・探索仕様](STORY.md)を参照。福ちゃん・やめ太郎・そば屋は正典Irodori、たこさんは第21話と同じVOICEVOX:Voidoll（style 89）。[音声台本と生成条件](../04_GAME_ASSETS/audio/hazard/script.md)を正本とし、ゲームから相対symlinkで利用する。
...
村入口と農場にたこさん商店。会話開始時にビール10個以上あれば隠し商品を表示し、10個で一度だけ交換する。ケース7×2マスが必要。購入時に自動装備、以降は持ち物または3キーで装備。構えると画面内で遮蔽物のないそば屋を自動ロックし、クリックで追尾弾を発射。弾数無限、連射間隔1.25秒。通常個体は一撃、HP900の巨大そば屋には300ダメージ。ロケットで倒した個体はビールを落とさない。専用プロップは `game_rocket_visuals.dart`、発射・爆発音は `tools/build_hazard_rocket_audio.py` から再生成可能。

最高難度では全個体を撃破するまで次章・脱出へ進めない（前章への引き返しは可）。初期は装填2発・予備0発・ショットガン0発。やめ太郎の一度限りの補給10発、各章に散在する弾は1発のみ、箱から弾は出ない。たこさんの弾薬はビールと引き換えに補充可能。通常個体HP100に対するダメージは以下。頭部は骨格、ジョッキは持ち手の変換に追従する球で判定し、ショットガンの広い胴体判定では弱点ボーナスを与えない。
...
最高難度の蹴りは8ダメージで連続怯ませ不可。無料3発だけでは倒せるのは最大1体（ビール1個で交換最低価格2個に届かない）。やめ太郎の10発込みでも村8体分に足りず、交換が必要。通常個体19体の最少57発とボス頭部へのショットガン9発に対し、初期・補給・配置16発＋弾10発×5回（ビール10個）＋散弾5発×2回（6個）で収支が成立する。これは命中を前提にした資源検証で、人間の通しプレイによる難度評価は別途必要。
