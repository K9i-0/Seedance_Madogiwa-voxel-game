...
2026-09-17 ユーザー確認済みの静的仕上げ工程の記録。その後 `../rig_v3_20260917/` で姿勢・リグ・モーションを追加し、ゲームと公式サイトへ採用済み。制作全体の知見は `04_GAME_ASSETS/3d/SOBAYA_V3_LESSONS.md`。

## 顔サイズ・赤ライン修正版（2026-09-17、本番採用）

最新正本は `face_red_edge_20260917/sobaya_front_matched.blend` と同名GLB。仮面外周を固定し、顔パーツを90%に縮小したうえで赤ラインを目の縁まで延長。これはリグ前の静的正本。現在の公式サイトは `../rig_v3_20260917/sobaya_rig.glb` を参照。再読込して正面・斜め・側面・全身を確認済み。

## 前回の顔パーツ修正版（保存版）

前回の顔の修正は `face_refinement_20260917/` に保存。
...
`inputs/front.png` を基準に、目・口・額の丸の比率と赤い模様を調整し、鼻孔のない鼻先へ修正。ユーザー指示により輪郭と髪は元のv3を維持。
