...
身長1.8m、55,398三角面、23ボーン、約12.1MB。BlenderではBody / Headの2メッシュで、GLBでは補修材質を含め5描画メッシュとなる。髪はHeadに含まれる。仮面は硬い形を保ち、表情モーフは設けていない。

目と口には、仮面のすぐ裏へ浅い黒い面を追加した。深い空洞を埋め、既存の内側も黒に統一。縁の近くでは仮面の形に沿わせてはみ出しを抑え、GLBの `KHR_materials_unlit` で照明・反射による光沢をなくす。追加面はHeadと結合し、頭ボーンに100%追従する。実装は `tools/sobaya_v2_mask.py`、顔アップは [qa/mask_front.png](qa/mask_front.png) と [qa/mask_left_oblique.png](qa/mask_left_oblique.png)（ローカル保持）。
...
目と口の黒い裏面の修正はローカルのBlender処理で、追加クレジット消費は0。
