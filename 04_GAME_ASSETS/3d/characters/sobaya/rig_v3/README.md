...
腰・裾のウェイト改訂では、v2正本からv3を生成する際に `sobaya_garment.relax_hem_weights` を一度適用する。腰周り891頂点の隣接ウェイトを滑らかにつなぎ、後ろの裾とズボンの腰を骨盤へ追従させる。メッシュ・UV・骨数は増やしていない。旧版で裾が尖って伸びた問題を改善し、窓越えの腰の最大辺伸長率は52.61倍→4.40倍。11クリップ・各モデル1167姿勢の検査、GLB再読込描画、Flutterでの敵の窓越え追跡を確認。詳細と残る伸縮は `21_SOBAYA_HAZARD_LAB/qa/sobaya-garment-20260906.json` を参照。
