"""Import the recorded whole-body output, normalize to 1.70m, and inspect it.
...
bpy.ops.export_scene.gltf(filepath=str(OUT / 'fukuchan_wholebody.glb'), export_format='GLB', use_selection=True, export_animations=False)
record = {'source': str(source.relative_to(OUT)), 'mesh_objects': len(meshes),
          ...
          'height_m': 1.7, 'rigged': False, 'materials': materials}
...
bpy.ops.object.select_all(action='SELECT')
bpy.ops.object.delete(use_global=False)
bpy.ops.import_scene.gltf(filepath=str(OUT / 'fukuchan_wholebody.glb'))
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT / 'fukuchan_wholebody.blend'))
print('FUKUCHAN_WHOLEBODY_REVIEW_DONE', flush=True)
