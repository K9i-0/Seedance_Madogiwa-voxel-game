...
def author(rig,mesh):
 ...
 # Turn the pelvis/legs together around the planted right ankle. Unwind the
 # torso gradually so the chest, face and fists remain front-facing.
 chest_before=rig.pose.bones['Spine2'].head.copy()
 pivot=rig.pose.bones['RightFoot'].head.copy()
 hip.matrix=Matrix.Translation(pivot)@Matrix.Rotation(math.radians(-35),4,'Z')@Matrix.Translation(-pivot)@hip.matrix
 bpy.context.view_layer.update()
 for name,degrees in [('Spine',12),('Spine1',13),('Spine2',10)]:
  b=rig.pose.bones[name];b.matrix=Matrix.Translation(b.head)@Matrix.Rotation(math.radians(degrees),4,'Z')@b.matrix.to_3x3().to_4x4();bpy.context.view_layer.update()
 chest_shift=rig.pose.bones['Spine2'].head-chest_before
 for side,sign,short in [('Left',1,'l'),('Right',-1,'r')]:
  goal=Vector((sign*.145-.095,-.19,1.34))+chest_shift;body.ik(['upperarm_'+short,'lowerarm_'+short,'hand_'+short],goal,Vector((sign*.1,-.25,-1)))
