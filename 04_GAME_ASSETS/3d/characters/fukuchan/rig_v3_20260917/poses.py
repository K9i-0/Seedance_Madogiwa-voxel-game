...
def author(rig,mesh):
 ...
 reset()
 # Photo: subject's right knee lifts, both fists beside the cheeks, elbows down.
 hip=rig.pose.bones['Hips'];h=hip.matrix.copy();h.translation+=Vector((.065,0,-.02));hip.matrix=h;bpy.context.view_layer.update()
 body.leg_ik('l',Vector((.13,.032,.105)),body.rest['LeftFoot'].to_3x3())
 thigh=rig.pose.bones['RightUpLeg'];knee=thigh.head+Vector((-.50,-.85,.12)).normalized()*body.length('thigh_r','calf_r');body.aim(thigh,knee);calf=rig.pose.bones['RightLeg'];body.aim(calf,calf.head+Vector((.25,-.02,-1)));foot=rig.pose.bones['RightFoot'];foot.matrix=Matrix.Translation(foot.head)@body.rest['RightFoot'].to_3x3().to_4x4();bpy.context.view_layer.update()
 ...
 h=hip.matrix.copy();h.translation.z+=.003-body.skin_floor('l');hip.matrix=h;bpy.context.view_layer.update()
