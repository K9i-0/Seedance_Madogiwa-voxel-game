...
MODE=sys.argv[sys.argv.index('--')+1] if '--' in sys.argv else 'shallow'
path=OUT.parent/'imagegen_texture_20260917/fukuchan_texture_fixed.glb' if MODE=='baseline' else OUT/('fukuchan_'+MODE+'.glb')
...
for name,d in [('front',(0,-1,0)),('oblique',(.6,-1,0)),('profile',(1,0,0))]:
    if '--albedo' in sys.argv and name!='front':continue
    cam.location=target+Vector(d).normalized()*5;cam.rotation_euler=(target-cam.location).to_track_quat('-Z','Y').to_euler();sc.render.filepath=str(QA/(MODE+'_'+name+'.png'));bpy.ops.render.render(write_still=True)
...
if '--full' in sys.argv:
    ...
    for name,d in [('full_front',(0,-1,0)),('full_oblique',(.6,-1,0)),('full_back',(0,1,0)),('right_profile',(-1,0,0)),('badge',(0,-1,0))]:
        ...
        sc.render.filepath=str(QA/(MODE+'_'+name+'.png'));bpy.ops.render.render(write_still=True)
