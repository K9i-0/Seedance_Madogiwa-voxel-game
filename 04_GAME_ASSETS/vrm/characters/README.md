...
ゲームの共通GLBから再生成するVRM 1.0対応版。既存の顔・体格・メッシュ・テクスチャ・スキンウェイト・初期姿勢・骨名を維持し、VRM Humanoidの役割を明示する。ゲーム用GLBは引き続き `04_GAME_ASSETS/3d/motion_library/` が正本。
...
VRM 1.0ではglTFのanimationsは使用対象外のため、このVRMのJSONから削除する。既存143比較クリップとゲーム用クリップは元GLBに保持。再エンコードによる形状劣化を防ぐため元のバイナリチャンクを保持し、未参照となるアニメーションデータの圧縮・除去は行っていない。manifestに正本と出力のSHA-256を記録。

## 再生成と検証

```sh
python3 tools/export_humanoid_vrm.py
