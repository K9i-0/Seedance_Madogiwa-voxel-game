...
- prop source：`prop_bug_deity_yametaro_statue_production.png`。
- 用途：9.7秒のハードカット直後へ完全一致させる、ギュンギュン斉唱の本番開始フレーム。
...
- 用途：4.0〜14.2秒の画面下部へ直接合成する透明PNG。14.2秒の石像アップでは消す。
...
- prop source：`prop_bug_deity_yametaro_statue_production.png`。
- 用途：14.2秒の最終ハードカット直後へ完全一致させる、14.2〜15.0秒の0.8秒だけ使う石像アップの本番開始フレーム。
...
- 用途：14.2〜15.0秒の0.8秒だけ直接合成する、控えめな文化ニュース風の解説叩き。透明PNG。
