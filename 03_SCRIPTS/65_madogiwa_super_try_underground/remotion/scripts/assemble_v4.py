"""Reproduce v4 assembly without overwriting any v1/v2 asset."""
import argparse
import os
...
args = p.parse_args()
part1 = r / 'wan3_part1_v4_480p.mp4'
part2 = r / 'wan3_part2_v4_480p.mp4'
...
if args.mux_final:
    subprocess.run(['ffmpeg','-v','error','-i',str(r/'remotion/out/visual_v4.mp4'),'-i',str(part1),'-i',str(part2),'-filter_complex',audio,'-map','0:v','-map','[a]','-c:v','copy','-c:a','aac','-b:a','192k','-t','45','-movflags','+faststart','-y',str(r/'final_remotion_cm_v4.mp4')],check=True)
else:
    output = r/'joined_wan_v4_480p.mp4'
    ...
    target = r/'remotion/public/input_v4.mp4'
    if target.exists():
        target.unlink()
