...
try{
 const composition=await selectComposition({serveUrl,id:(mode==='full'||mode==='audit')?'MadogiwaPrisonBreak':'PrisonBreakEndcard',puppeteerInstance:browser});
 if(mode==='audit'){
 ...
 }else await renderMedia({serveUrl,composition,puppeteerInstance:browser,codec:'h264',pixelFormat:'yuv420p',outputLocation:mode==='full'?'../final_remotion_prison_break.mp4':'out/endcard-preview.mp4'});
