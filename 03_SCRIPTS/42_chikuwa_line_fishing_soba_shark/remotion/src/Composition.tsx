...
import type {
  ...
  VarietyPopup as VarietyPopupType,
} from './types';
...
const VarietyPopupComponent: React.FC<{overlay: VarietyPopupType}> = ({overlay}) => {
  const frame = useCurrentFrame();
  ...
  // 微細な脈動アニメーション（バラエティ感を強調）
  const pulse = Math.sin(frame * 0.18) * 0.025 + 1.0;
  ...
  const scale = enterSpring * pulse * exitScale;
  const opacity = exitOpacity;
  const initialRotate = overlay.rotateDeg ?? -3;
  const currentRotate = interpolate(frame, [0, 10], [initialRotate, 0], {
    extrapolateRight: 'clamp',
  });

  const isBottom = overlay.bottom !== undefined;
  return (
    <div
      style={{
        position: 'absolute',
        ...(isBottom ? {bottom: overlay.bottom} : {top: overlay.top ?? '15%'}),
        left: overlay.left ?? '50%',
        transform: `translate(-50%, 0) scale(${scale}) rotate(${currentRotate}deg)`,
        opacity,
        transformOrigin: isBottom ? 'center bottom' : 'center center',
        zIndex: 30,
...
const CaptionLayer: React.FC<{
  ...
  const {height} = useVideoConfig();
  const fontSize = Math.max(16, Math.round(height * 0.058));
  const textColor = caption.color ?? 'white';
  ...
  return (
    <div
      style={{
        ...
        lineHeight: 1.35,
        WebkitTextStroke: `${Math.max(2, fontSize * 0.1)}px rgba(0,0,0,0.95)`,
    ...
    >
      {caption.speaker ? (
        <span style={{fontSize: '0.8em', opacity: 0.9, marginRight: 8}}>
          [{caption.speaker}]
        </span>
      ) : null}
      {caption.text}
...
const renderOverlay = (overlay: Overlay, hasTicker: boolean) => {
  ...
  if (overlay.type === 'variety-popup') {
    return <VarietyPopupComponent overlay={overlay} />;
  }
...
export const EditComposition: React.FC<{manifest: EditManifest}> = ({manifest}) => {
  ...
  const activeCaptions = manifest.captions.filter(
  ...
  return (
    <AbsoluteFill style={{backgroundColor: '#000'}}>
      ...
      {activeCaptions.map((caption, i) => (
        <CaptionLayer
          key={`caption-${caption.startFrame}-${i}`}
          caption={caption}
          elevated={hasActiveLowerThird}
        />
      ))}
    </AbsoluteFill>
  );
