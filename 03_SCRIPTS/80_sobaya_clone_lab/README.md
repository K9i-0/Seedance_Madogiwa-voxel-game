## 現行候補：短い暗転→ドゥーン→タイトルコール

`final_remotion_clone_lab_title_boom.mp4`（約33.3秒）。最後のセリフの後に0.27秒の暗転・無音、重低音と短い金属音でA背景＋タイトルを表示。小さな揺れのあと、0.4秒遅れてそば屋のコール。[締めを比較](review.html)。

## 現行：やめ太郎を先に測定

`final_remotion_clone_lab_scan_order.mp4`（33秒）。測定画面は左がやめ太郎、右が福ちゃん。5,082は375f（12.50秒）で確定し、「五千」の音声配置384f（12.80秒）より9フレーム先行。福ちゃんは後から213へカウントアップ。音声・反響・A背景タイトルは前回のまま。[比較ページ](review.html)。

現行UIのタイミング正本は `remotion/src/edit-manifest.json`。既存の単体v2プレビューは履歴として保持し、現行コードは今回の順序を再現する。

## 現行：静かな培養室・会話の反響・A背景タイトル

`final_remotion_clone_lab_quiet_lab.mp4`（33秒）。序盤の二人の会話を背景から分離して室内反響を追加。警報前は小さな培養音を背景にし、追加のガラス打音は撤去。ラストは採用A案の研究所画像へタイトルを重ねる。Aのたこさん音声・測定人物の静止化・そば屋のタイトルコールは維持。[比較ページ](review.html)。
