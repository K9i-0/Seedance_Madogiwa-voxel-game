...
右上の調整アイコン、またはEscから設定パネル。景色を表示したまま変更でき、移動入力は解除・遮断する。Esc／×／パネル外クリックで閉じる。

- 朝・昼・夕暮れ・夜のプリセットと24時間スライダー。時間経過は1日5〜120分。背景化時は止める。
- 晴れ・曇り・霧・雨・嵐、天気の強さ、風、環境音量。時間／天気は約1〜2秒で滑らかに切り替わる。
- 太陽／月の向き・色・影、空のグラデーションと星、多層ノイズ雲、距離・高さ霧、海の色・反射・波・雨の波紋を連動。空からの環境光は64px面／256px横幅で3秒間隔に再計算し、以降の更新はfork側がフレーム分散する。反映には短い遅れがある。
- 既存街灯16か所に暖色の点光源と発光部を追加。雨で粗い材質を暗く滑らかにし、止むと徐々に乾く。
- カメラ周辺の雨粒を1メッシュでGPUアニメーション。地形／屋根上面でクリップする。風の横ずれや樹木下の遮蔽は近似で、物理的な雨滴衝突や水たまりではない。
- 鳥は夜・雨で抑制。風の強さに風音が連動。雨ループと間隔を空けた稲光／遅れて鳴る遠雷を追加。新規音は[共通正本](../04_GAME_ASSETS/audio/weather/README.md)の自作合成音。追加ネットワーク取得なし。
- 高画質: renderScale1、反射0.45、影2048、雨484。軽量: 0.75／0.25／1024／256。旧「海の改善」節の反射0.35は時間天気導入前の値。
- 設定はApplication Supportの`island-atmosphere.json`へdebounce＋一時ファイルrenameで保存。破損値は初期値／範囲内へ復旧し、保存エラーはパネルへ表示。

`madogiwa.setAtmosphere`で`hour`、`weather=clear|cloudy|mist|rain|storm`、`strength`、`wind`、`autoTime`、`dayMinutes`、`quality=balanced|high`、`volume`を設定。`inspectTidewater.atmosphere`で設定／実際の時刻／雨／濡れ量／灯数を取得。ベンチマークは保存設定を無視し、`WEATHER_SCENARIO`（既定clear）、`WEATHER_HOUR`（既定13）で固定条件を指定する。

雲は多層の平面ノイズ近似で、ボリューム雲のレイマーチではない。原作のプロシージャル木材・岩・植生は引き続き未移植。Flutter Scene fork／Flutter GPU／SDKの追加改修なし。[実機検証とprofile結果](qa/atmosphere-20260926/README.md)。
