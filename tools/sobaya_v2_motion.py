"""Retarget Sobaya v1 motions to the new Tripo body; author five review actions.

The source skeleton is mapped anatomically, including its two-segment spine.
The target uses Mixamo names, hence Body(..., name="fukuchan") selects
that naming convention only; no Fukuchan character asset is imported.
"""
import math,hashlib
import bpy
from mathutils import Matrix,Quaternion,Vector
from sobaya_v2_common import ROOT
from build_humanoid_motion import Body,use_action,clear_pose

SOURCES={
 'Idle':'Adopted_Library_Idle_A','Walk':'Adopted_Library_Walk','Run':'Adopted_Candidate_Chase_Run',
 'DanceStep':'Adopted_Library_Dance_Simple','DanceDisco':'Adopted_Library_Dance_Charleston','DanceVictory':'Adopted_Library_Dance_Body_Roll',
}

# Convert v1 Sobaya anatomical names into the new Tripo Mixamo naming.
BONE_NAMES={'Root':'Root','Hips':'Hips','Spine':'Spine','Chest':'Spine2','Neck':'Neck','Head':'Head'}
for side,word in [('L','Left'),('R','Right')]:
 for old,new in [('Shoulder','Shoulder'),('UpperArm','Arm'),('Forearm','ForeArm'),('Hand','Hand'),('Thigh','UpLeg'),('Shin','Leg'),('Foot','Foot'),('Toe','ToeBase')]:
  BONE_NAMES[old+'.'+side]=word+new

def adapt_matrices(matrices):
 result={new:matrices[old].copy() for old,new in BONE_NAMES.items()}
 low,high=result['Spine'],result['Spine2']
 q=low.to_quaternion().slerp(high.to_quaternion(),.5)
 result['Spine1']=Matrix.Translation(low.translation.lerp(high.translation,.5))@q.to_matrix().to_4x4()
 return result

def read_motion_sources():
    source=ROOT/'04_GAME_ASSETS/3d/hazard_adopted/sobaya.glb'
    ...
    rest=adapt_matrices({b.name:bind@b.matrix_local for b in rig.data.bones})
    record={}
    for name,label in SOURCES.items():
        ...
        for i in range(frames+1):
            ...
            samples.append(adapt_matrices({b.name:rig.matrix_world@b.matrix for b in rig.pose.bones}))
    ...
    for name in ['Greeting','Test_HeadTurn','Test_ArmRaise','Test_ElbowBend','Test_KneeBend']:
        ...
        for i in range(frames+1):
            ...
            if name=='Greeting':
                ...
                goal=h.head.lerp(Vector((-.46,-.20,1.62)),envelope)
                ...
                for side,sign in [('l',1),('r',-1)]:
                    a=body.bone('upperarm_'+side);angle=math.radians(145)*envelope
                ...
                for side,sign in [('l',1),('r',-1)]:
                    h=body.bone('hand_'+side);goal=h.head.lerp(Vector((sign*.29,-.36,1.44)),envelope)
