...
from fukuchan_head_assembly import replace_head
head, report['head_assembly'] = replace_head(mesh, rig)
report['speech_shapes'] = report['head_assembly']['speech_shapes']
(OUT/'.gitignore').write_text('*.blend\n*.blend1\n')
(OUT/'rig.json').write_text(json.dumps(report,indent=2)+'\n')
bpy.ops.object.select_all(action='DESELECT')
for o in [rig,mesh,head,socket]:o.select_set(True)
