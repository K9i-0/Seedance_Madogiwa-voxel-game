...
const expected=['Idle','Walk','Run','DanceStep','DanceDisco','DanceVictory','Greeting','Test_HeadTurn','Test_ArmRaise','Test_ElbowBend','Test_KneeBend'];
assert.deepEqual(gltf.animations.map(a=>a.name).sort(),expected.sort());
for(const name of ['Body','Head'])assert(gltf.scene.getObjectByName(name),'Missing separate part '+name);
...
assert(Math.abs(restBox.max.y-restBox.min.y-1.8)<.002,JSON.stringify(restBox));
...
assert.deepEqual(Object.keys(morphs),[], 'Rigid mask should have no face morphs');
assert(!gltf.scene.getObjectByName('Hair'),'Hair must remain part of Head');
