...
variant='--rig-highlight' in sys.argv
clip='WanRig_Walk' if variant else 'Wan_Walk'
slug='wan-rig-walk' if variant else 'wan-walk'
folder=ROOT/('22_HUMANOID_MOTION_LAB/evidence/'+slug);folder.mkdir(parents=True,exist_ok=True)
report={'characters':[]}
for name in ['sobaya','fukuchan']:
 ...
 use_action(rig,bpy.data.actions[clip]);floors=[];pitches=[];positions={s:[] for s in ['l','r']}
 profile=json.loads((OUT/name/'profile.json').read_text());speed=next(c for c in profile['clips'] if c['name']==clip)['groundSpeedMps']
 frames=round(bpy.data.actions[clip].frame_range[1]);samples=frames*4
 ...
 for s,offset in [('l',0),('r',.5)]:
  for i in range(1,samples):
   a=((i-1)/samples+offset)%1;b=(i/samples+offset)%1
   if .12<a<b<.48:
    v=(positions[s][i]-positions[s][i-1])*120-Vector((0,speed,0));errors.append(math.hypot(v.x,v.y))
 item=dict(character=name,samples=samples+1,clip=clip,minSoleM=min(floors),maxLowestSoleM=max(floors),gazePitchRangeDeg=[min(pitches),max(pitches)],midStanceAnkleSlipRmsMps=math.sqrt(sum(x*x for x in errors)/len(errors)),groundSpeedMps=speed)
 ...
 for i in range(8):
  sc.frame_set(round(i*frames/8));sc.render.filepath=str(folder/f'{name}_{i}.png');bpy.ops.render.render(write_still=True)
(ROOT/f'22_HUMANOID_MOTION_LAB/qa/{slug}-20260907.json').write_text(json.dumps(report,indent=2)+'\n')
