...
画像・動画制作では[顔固定の共通文](references/yametaro-identity-prompt.md)を読み、実際に送るプロンプトへ要点を省略せず展開する。参照シートの指定だけで済ませず、白いレンズ、瞳・まぶたの追加防止、頭部・頭髪の比率、鼻・口・体格を固定する。怒りや目覚めも造形を変えずに演じる。ユーザーが明示した外見変更は優先する。
