...
福ちゃんの3Dビュー・AR共通モデルは `characters/fukuchan/web_v3_20260917/fukuchan.glb`。首補修・浅い顔形状・54骨・20動作・交差脚と腰ひねりのギュンギュンを含む。`build.mjs` でリグ正本からテクスチャを可逆WebPへ変換し、全画素一致・形状と動作バッファ保持を検証。24,244,864 bytes。ローカル試作の `public/.local/` は `.assetsignore` で配信対象外。モデルURLの世代を `rig-v3-even-skin-20260917` へ更新した。
...
2026-09-17: 福ちゃんv3は `skin_even_20260917` の頬補正版を配信入力に採用。正面の明るい肌色を基準に、左右の頬の焼き込み陰影を薄くした。形状・骨・20動作は保持。詳細は同フォルダREADME。
...
`character-3d-focus.test.ts` は短い2連タップ、capture解放、ドラッグ、ピンチ／2本指操作、キャンセル、長押し、画角、拡縮・ポーズ適用済みの頭の範囲を検証。Chromeで4キャラの顔アップ、福ちゃんの手のダブルクリック拡大・全身復帰、390px幅の顔アップとボタン配置を確認。スマホ実機のマルチタッチ操作は未確認（ジェスチャー判定はユニットテスト、2本指の移動とピンチはOrbitControlsの標準操作を使用）。
