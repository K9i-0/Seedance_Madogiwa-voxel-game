...
福ちゃんの3Dビュー・AR共通モデルは `characters/fukuchan/web_v3_20260917/fukuchan.glb`。首補修・浅い顔形状・54骨・20動作・交差脚と腰ひねりのギュンギュンを含む。`build.mjs` でリグ正本からテクスチャを可逆WebPへ変換し、全画素一致・形状と動作バッファ保持を検証。24,183,360 bytes。ローカル試作の `public/.local/` は `.assetsignore` で配信対象外。モデルURLの世代を `rig-v3-gyun-twist-20260917` へ更新した。
