...
- 公式UIはroot loaderの `getOfficialShell` で公開一覧・ギャラリー・テーマCookieを取得。子ルートの旧UI用loaderは実行せず、記事データも取得しない。Cookieを含む応答は共有キャッシュに保存しない。
- 詳細ページの主動画・別バージョンは `DeferredVideo` で再生クリック時だけvideo要素を作る。関連カードは遅延読み込みの画像。動画URLとVideoObjectのSEOメタデータは維持。
- リンクの一律先読みを無効化。検索・絞り込み・テーマ切り替えは取得済みデータをブラウザーで処理する。
