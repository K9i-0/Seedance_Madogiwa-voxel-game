...
import { modelCharacters, cameraUrl, type ARCharacter } from "./character-ar-config";
...
export function CharacterCameraPage({ character }: { character: ARCharacter }) {
  return <div className="j-camera-shell"><main className="j-model-dialog j-camera">
    ...
    <nav className="j-camera-characters" aria-label="撮影するキャラクター">
      {modelCharacters.map(({ id, name }) => <a key={id} href={cameraUrl(id)} aria-current={character === id ? "page" : undefined}><img src={`/site/characters/${id}.webp`} alt=""/><span>{name}</span></a>)}
    </nav>
