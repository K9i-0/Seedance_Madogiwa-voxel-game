...
export function CharacterAR({ character }: { character: string }) {
  ...
  return <section className="j-ar" aria-label="窓際ARカメラ">
    ...
    {id === "sobaya" && <div className="j-ar-poses"><label><input type="checkbox" checked={mug} onChange={(event) => setMug(event.target.checked)} /> ジョッキを持つ</label></div>}
    ...
    <ARLaunch key={`${id}:${placement}:${pose}:${mug}`} character={id} placement={placement} pose={pose} mug={id === "sobaya" && mug} />
...
function ARLaunch({ character, placement, pose, mug }: { character: ARCharacter; placement: ARPlacement; pose: "Idle" | "Wave"; mug: boolean }) {
  ...
  const prepare = async () => {
    ...
    try {
      ...
      const blob = await createARAsset(character, placement, pose, controller.signal, mug);
      controller.signal.throwIfAborted();
