...
function VideoForm({ generation, onSaved }: { generation: Generation; onSaved: () => Promise<EpisodeDetail> }) {
  ...
  return <div className="space-y-4"><Card><CardHeader><h3 className="text-sm font-medium">生成 v{generation.version}の動画</h3><p className="text-xs text-stone-600">イチオシにすると公式サイトの最新動画や絞り込みで優先されます。</p></CardHeader><CardContent><form onSubmit={submit} className="space-y-5"><Field label="表示名"><Input name="label" defaultValue={defaultLabel} required /></Field><label className="flex cursor-pointer items-center gap-3 rounded-xl border border-amber-300/15 bg-amber-300/[0.04] px-4 py-3 text-sm text-stone-300"><input type="checkbox" name="featured" className="accent-amber-400" /><Star className="size-4 text-amber-300" />イチオシ動画として登録</label><UploadPicker file={file} label="動画を選択" accept="video/*" onFile={setFile} /><Button disabled={!file || uploading}><CloudUpload className="size-4" />{uploading ? "R2へアップロード中…" : "動画を登録"}</Button></form></CardContent></Card>{generation.videos.length ? <Card><CardContent className="space-y-2 pt-6">{generation.videos.map((video) => <div key={video.id} className="flex items-center justify-between gap-4 rounded-xl bg-white/[0.025] px-4 py-3"><div className="flex min-w-0 items-center gap-3"><FileVideo2 className="size-4 shrink-0 text-stone-600" /><span className="truncate text-sm text-stone-300">{video.label}</span>{video.is_featured ? <Badge className="border-amber-300/20 bg-amber-300/10 text-amber-200"><Star className="mr-1 size-3" fill="currentColor" />イチオシ</Badge> : null}</div><div className="flex items-center gap-2"><Button type="button" variant="ghost" size="sm" onClick={() => void toggleFeatured(video.id, !video.is_featured)}><Star className={cn("size-3.5", video.is_featured ? "fill-amber-300 text-amber-300" : "text-stone-500")} />{video.is_featured ? "解除" : "イチオシ"}</Button><StatusBadge status={video.status} /></div></div>)}</CardContent></Card> : null}</div>;
}
